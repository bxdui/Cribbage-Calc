...
class CribbageCalc:
    ...
    def count_15s(hand, quantity):
        ...
        com_lst = [x + y for x in val_lst for y in val_lst]
        op_lst = val_lst + com_lst

        score = []

        if 5 in op_lst and 10 in op_lst:
            score.append(2)

        if 9 in op_lst and 6 in op_lst:
            score.append(2)

        if 7 in op_lst and 8 in op_lst:
            score.append(2)
